...
internal_ball_on <= '1' when ( ('0' & ball_x_pos <= '0' & pixel_column + size) and ('0' & pixel_column <= '0' & ball_x_pos + size) 	-- x_pos - size <= pixel_column <= x_pos + size
					and ('0' & ball_y_pos <= pixel_row + size) and ('0' & pixel_row <= ball_y_pos + size) )  else	-- y_pos - size <= pixel_row <= y_pos + size
			'0';

-- Output ball_on signal
ball_on <= internal_ball_on;

-- Colours for pixel data on video signal
-- Red ball with white background
Red <=  "1111" when internal_ball_on = '1' else "0000";
Green <= "0000" when internal_ball_on = '1' else "1111";
Blue <=  "0000" when internal_ball_on = '1' else "1111";
